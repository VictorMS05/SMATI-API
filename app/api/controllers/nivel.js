...
function consultar_nivel(req, res) {
    ...
    switch (periodo_de_tiempo) {
        case 'reciente':
            ...
            break;
        case 'tiempo_real':
            consulta_sql = 'SELECT x.nombre, CONCAT(EXTRACT(HOUR FROM n.fecha), \':\', EXTRACT(MINUTE FROM n.fecha), \':\', EXTRACT(SECOND FROM n.fecha)) AS tiempo, n.nivel, r.altura FROM nivel AS n INNER JOIN xbee AS x ON n.id_xbee = x.id_xbee INNER JOIN riesgo AS r ON n.nivel = r.nivel WHERE n.id_xbee = $1 AND n.fecha >= NOW() - INTERVAL \'121 seconds\' ORDER BY n.fecha DESC LIMIT 60;';
            break;
        case '24_horas':
            consulta_sql = 'SELECT x.nombre, DATE_TRUNC(\'hour\', n.fecha) AS hora, AVG(n.nivel) AS promedio_nivel, CONCAT(AVG(n.nivel)*30, \' cm\') AS promedio_altura FROM nivel AS n INNER JOIN xbee AS x ON n.id_xbee = x.id_xbee WHERE n.id_xbee = $1 n.fecha >= NOW() - INTERVAL \'24 hours\' GROUP BY DATE_TRUNC(\'hour\', fecha) ORDER BY hora DESC LIMIT 24;';
            break;
        case '7_dias':
            consulta_sql = 'SELECT x.nombre, DATE_TRUNC(\'day\', n.fecha) AS dia, AVG(n.nivel) AS promedio_nivel, CONCAT(AVG(n.nivel)*30, \' cm\') AS promedio_altura FROM nivel AS n INNER JOIN xbee AS x ON n.id_xbee = x.id_xbee WHERE n.id_xbee = $1 n.fecha >= NOW() - INTERVAL \'7 days\' GROUP BY DATE_TRUNC(\'day\', fecha) ORDER BY dia DESC LIMIT 7;';
            break;
        case '30 dias':
            consulta_sql = 'SELECT x.nombre, DATE_TRUNC(\'day\', n.fecha) AS dia, AVG(n.nivel) AS promedio_nivel, CONCAT(AVG(n.nivel)*30, \' cm\') AS promedio_altura FROM nivel AS n INNER JOIN xbee AS x ON n.id_xbee = x.id_xbee WHERE n.id_xbee = $1 n.fecha >= NOW() - INTERVAL \'30 days\' GROUP BY DATE_TRUNC(\'day\', fecha) ORDER BY dia DESC LIMIT 30;';
            break;
        case '12_meses':
            consulta_sql = 'SELECT x.nombre, DATE_TRUNC(\'month\', n.fecha) AS mes, AVG(n.nivel) AS promedio_nivel, CONCAT(AVG(n.nivel)*30, \' cm\') AS promedio_altura FROM nivel AS n INNER JOIN xbee AS x ON n.id_xbee = x.id_xbee WHERE n.id_xbee = $1 n.fecha >= NOW() - INTERVAL \'12 months\' GROUP BY DATE_TRUNC(\'month\', fecha) ORDER BY mes DESC LIMIT 12;';
            break;
        default:
    ...
    bd.query(consulta_sql, [id])
        .then(results => {
            if (results.rowCount > 0) { // Si hay registros
                console.log('GET /nivel/' + id + ' HTTPS/1.1 200 OK');
        ...
        .catch(error => {
            console.log('GET /nivel/' + id + ' HTTPS/1.1 500 Internal Server Error');
            json.error = true;
...
function insertar_nivel(req, res) {
    ...
    if (body.id_xbee == 0 || body.id_xbee == null || body.id_xbee == undefined) {
        console.log('POST /nivel HTTPS/1.1 400 Bad Request');
        json.error = true;
        ...
        bd.query('INSERT INTO nivel(id_xbee, nivel, fecha) VALUES($1, $2, CURRENT_TIMESTAMP) RETURNING *;', [body.id_xbee, body.nivel])
            .then(results => {
                if (results.rowCount > 0) { // Si se insertó el registro
                    console.log('POST /nivel HTTPS/1.1 201 Created');
            ...
            .catch(error => {
                console.log('POST /nivel HTTPS/1.1 500 Internal Server Error');
                json.error = true;
                json.mensaje = "Hubo un error en el servidor";
